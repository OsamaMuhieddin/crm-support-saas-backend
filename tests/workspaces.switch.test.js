...
describe('Workspace switching + explicit active workspace context', () => {
  maybeDbTest(
    ...
    async () => {
      ...
      expect(mineResponse.body.currentWorkspaceId).toBe(memberUser.workspaceId);
      const mineWorkspaceIds = mineResponse.body.memberships.map(
      ...
      mineResponse.body.memberships.forEach((item) => {
        expect(item.workspace._id).toBeUndefined();
      });
